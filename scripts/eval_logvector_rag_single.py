...
EMAIL_RE = re.compile(r'[A-Za-z0m9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}')
...
# Evaluation dataset
# -----------------------------
# The answer ID uses the row index (0, 1, 2...) of the CSV file.
EVAL_DATASET: List[Dict] = [
    ...
    },
    {
        ...
        "description": "함수-테이블 양방향 검색 (단일 정답)"
    },
    {
        "query": "wp_wc_admin_notes 테이블과 관련된 함수들을 모두 알려줘.",
        "expected_ids": ["3", "5"], # Row 3, 5는 wp_wc_admin_notes 접근 로그 (예시)
        "description": "테이블-함수 양방향 검색 (복수 정답)"
    },
    {
        "query": "우커머스의 관리자 노트 액션(wp_wc_admin_note_actions)과 관련된 데이터 흐름 정보는?",
        "expected_ids": ["4", "6"], # Row 4, 6은 wp_wc_admin_note_actions 접근 로그 (예시)
...
def main():
    ...
    print(f"[INFO] Total Chunks: {len(cards)} (Using Single-CARD)")
