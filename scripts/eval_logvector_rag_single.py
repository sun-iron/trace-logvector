...
- Evaluate the LogVector RAG retrieval performance of the Single-Card strategy.
- This is the most basic RAG strategy, using each CSV row as a single chunk.
- Comparative analysis is performed using the same query as the multi-grouping strategy (eval_logvector_rag_multi.py).
"""
...
# -----------------------------
# NOTE:
# - Single-chunk evaluation uses row-based IDs (e.g., "0", "1", "2") as ground-truth targets.
# - expected_ids represent the CSV row chunk(s) that contain the relevant evidence.
# - Unlike the multi-CARD setting, this baseline may require multiple rows to represent
#   a relational answer (e.g., table↔function relations), hence multiple expected_ids.

EVAL_DATASET: List[Dict] = [
    {
        # Table-centric query:
        # Goal: retrieve the row chunk that contains evidence about wp_usermeta access.
        "query": "Which functions access the wp_usermeta table?",
        "expected_ids": ["0"],  # Row 0: wp_usermeta, update_meta_cache(), ...
        "description": "Table-centric search (single answer / single row)"
    },
    {
        # Function-centric query:
        # Goal: retrieve the row chunk that mentions WP_Post::get_instance() and its accessed table(s).
        "query": "Which table(s) does the function WP_Post::get_instance() access?",
        "expected_ids": ["1"],  # Row 1: wp_posts, WP_Post::get_instance(), ...
        "description": "Function-centric search (single answer / single row)"
    },
    {
        # Bidirectional function–table query:
        # In the single-chunk baseline, the relation may be captured within one representative row.
        "query": "Which table(s) are used by the function AutomatticWAARTSDataStore::get_taxes()?",
        "expected_ids": ["2"],  # Row 2: wp_woocommerce_tax_rates, AutomatticWAARTSDataStore::get_taxes(), ...
        "description": "Bidirectional function–table search (single answer / single row)"
    },
    {
        # Table-centric with relational expansion:
        # Multiple rows may contain partial evidence of functions related to the same table.
        "query": "List all functions related to the wp_wc_admin_notes table.",
        "expected_ids": ["3", "5"],  # Rows 3 and 5 contain wp_wc_admin_notes access evidence (example)
        "description": "Table–function relational search (multiple answers / multiple rows)"
    },
    {
        # Keyword-based relational query:
        # The baseline may require multiple rows to capture 'data-flow' / relation evidence.
        "query": "What is the data-flow information related to the WooCommerce admin note actions (wp_wc_admin_note_actions)?",
        "expected_ids": ["4", "6"],  # Rows 4 and 6 contain wp_wc_admin_note_actions access evidence (example)
        "description": "Keyword-based relational search (multiple answers / multiple rows)"
    },
]
...
def main():
    ...
    ap.add_argument("--index-path", default="./data/logvector_cards_s.index", help="FAISS Index path")
    ap.add_argument("--chunks-path", default="./data/logvector_cards_s.pkl", help="Pickle path")
    ...
    print(f"[INFO] Total Chunks: {len(cards)} (Using Single-Chunk / Row-as-Chunk Baseline)")
    ...
    for item in EVAL_DATASET:
        try:
            q_embed = encode_query(item['query'], meta, args.model, args.normalize_sql, args.mask_pii)
    ...
    # 5. Output results
    print("\n" + "="*70)
    print("      Single-CARD RAG Retrieval Performance Metrics")
    print("="*70)
    print(f"  Embedding Type: {meta.get('type', 'N/A').upper()}")
    print(f"  Top-K Value: {args.top_k}")
    print(f"  Total Chunks: {len(cards)}")
    print("-" * 70)
